...
void exibirInterface(Fila* fc, Fila* fp, Pilha* p) {
    printf("\n========== INTERFACE ==========");
    printf("\nFila prioritária:\n");
    exibirProximosPacientes(fp, 4); // exibe proximos 4 pacientes da fila prioritaria
    printf("\nFila comum:\n");
    exibirProximosPacientes(fc, 4); // exibe proximos 4 pacientes da fila comum
    if (pilhaVazia(p)) {
        printf("\nÚltima consulta:\nHistórico vazio\n");
    }
    else {
    ...
    do {
        ...
        switch (opcao) {
            ...
            case 3: { // exibicao do historico
                printf("\n------------------------------\n");
                printf("Histórico: ");
                exibirHistorico(&historico, 10); // exibe ultimos 10 pacientes/consultas no historico
                printf("\n------------------------------\n");
                break;
            }
        }
